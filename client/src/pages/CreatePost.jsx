...
function CreatePost() {
  ...
  const generateImage = async() => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://imageaigeneration-mern-dall-e.onrender.com/api/v1/dalle', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
          body: JSON.stringify({ prompt: form.prompt })
          // 这个是输入到后端的body，定义了我输入后端的内容只有form.prompt
        })
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        const response = await fetch('https://imageaigeneration-mern-dall-e.onrender.com/api/v1/post', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const handleChange = (e) => {
    // state updater function usually created by the useState hook
